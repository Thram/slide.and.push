angular.module("slidePushMenu", []).factory('slidePush', ->
  ...
  return {
    slide: (menu, btn) ->
      ...
      (if menu.hasClass("spmenu-open") then menu.css("left", parseInt(menu.css("left")) - spmenuVerticalWidth) else menu.css("left", parseInt(menu.css("left")) + spmenuVerticalWidth))  if menu.hasClass("spmenu-left")
      (if menu.hasClass("spmenu-open") then menu.css("right", parseInt(menu.css("right")) - spmenuVerticalWidth) else menu.css("right", parseInt(menu.css("right")) + spmenuVerticalWidth))  if menu.hasClass("spmenu-right")
      (if menu.hasClass("spmenu-open") then menu.css("top", parseInt(menu.css("top")) - spmenuHorizontalHeight) else menu.css("top", parseInt(menu.css("top")) + spmenuHorizontalHeight))  if menu.hasClass("spmenu-top")
      (if menu.hasClass("spmenu-open") then menu.css("bottom", parseInt(menu.css("bottom")) - spmenuHorizontalHeight) else menu.css("bottom", parseInt(menu.css("bottom")) + spmenuHorizontalHeight))  if menu.hasClass("spmenu-bottom")
    ...
    push: (menu, btn) ->
      ...
      if menu.hasClass("spmenu-left")
        ...
        (if menu.hasClass("spmenu-open")
        then body.css("left", bodyLeft - spmenuVerticalWidth)
        else body.css("left", bodyLeft + spmenuVerticalWidth))
        (if menu.hasClass("spmenu-open") then menu.css("left", parseInt(menu.css("left")) - spmenuVerticalWidth) else menu.css("left", parseInt(menu.css("left")) + spmenuVerticalWidth))
      if menu.hasClass("spmenu-right")
        bodyLeft = parseInt(body.css("left"))
        bodyLeft = (if bodyLeft then bodyLeft else 0)
        (if menu.hasClass("spmenu-open") then body.css("left", bodyLeft + spmenuVerticalWidth) else body.css("left",
          bodyLeft - spmenuVerticalWidth))
        (if menu.hasClass("spmenu-open") then menu.css("right", parseInt(menu.css("right")) - spmenuVerticalWidth) else menu.css("right", parseInt(menu.css("right")) + spmenuVerticalWidth))
      if menu.hasClass("spmenu-top")
        bodyTop = parseInt(body.css("top"))
        bodyTop = (if bodyTop then bodyTop else 0)
        (if menu.hasClass("spmenu-open") then body.css("top", "auto") else body.css("top", bodyTop + spmenuHorizontalHeight))
        (if menu.hasClass("spmenu-open") then menu.css("top", parseInt(menu.css("top")) - spmenuHorizontalHeight) else menu.css("top", parseInt(menu.css("top")) + spmenuHorizontalHeight))
      if menu.hasClass("spmenu-bottom")
        bodyTop = parseInt(body.css("top"))
        bodyTop = (if bodyTop then bodyTop else 0)
        (if menu.hasClass("spmenu-open") then body.css("top", "auto") else body.css("top", bodyTop - spmenuHorizontalHeight))
        (if menu.hasClass("spmenu-open") then menu.css("bottom", parseInt(menu.css("bottom")) - spmenuHorizontalHeight) else menu.css("bottom", parseInt(menu.css("bottom")) + spmenuHorizontalHeight))
      menu.toggleClass "spmenu-open"
...
).directive("ngSlideMenu",['slidePush', (slidePush) ->
  restrict: "A"
...
]).directive("ngPushMenu", ['slidePush', (slidePush)->
  restrict: "A"
  link: (scope, elem, attrs) ->
...
]).directive "ngSlidePushMenu", ["$document", 'slidePush', ($document, slidePush) ->
    ...
    link.transclude scope, (clone) ->
      ...
      if attrs.button
        ...
        if attrs.button is "slide"
          $document.mouseup (e) ->
            if not elem.is(e.target) and elem.has(e.target).length is 0 and not body.hasClass('modal-open')
              slidePush.slideForceClose(elem, btn)
        ...
        if attrs.button is "push"
          ...
          $document.mouseup (e) ->
            if not elem.is(e.target) and elem.has(e.target).length is 0 and not body.hasClass('modal-open')
              slidePush.pushForceClose(elem, btn)
          btn.click ->
